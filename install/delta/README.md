...
| Cray `cc`/`CC` compiler wrappers | conda-forge `openmpi` package's `mpicc`/`mpicxx` |
...
4. **Transient PMIx failures**: occasionally `srun` logs `No PMIx server
   was reachable... N singletons will be started` and the job hangs
   silently to the time limit (each rank thinks it's a lone MPI process of
   size 1, so nothing ever completes). This appears to be an intermittent
   per-node issue, not a systemic config problem -- verified via a
   standalone `mpi4py` rank/size sanity check that passed cleanly both
   before and after a run that hit this. If you see that message in the
   log, just resubmit.
...
| `fusion_distribution` (CPU) | 5000 particles, `n=32` MPI ranks | **468.3 s** | 84 s (06.11.25) at `n=128` | **Not apples-to-apples** -- Delta ran at 1/4 the rank count (see quota gotcha above), so each rank did ~4x the particles. Rerun at `n=128` if quota allows for a like-for-like number. |
