...
class BoozerRadialInterpolant(BoozerMagneticField):
    ...
    def __init__(
        ...
        # Broadcast values from root process to all processes (MPI) or assign directly (serial)
...
class InterpolatedBoozerField(sopp.InterpolatedBoozerField, BoozerMagneticField):
    ...
    def __init__(
        ...
        r"""
        Args:
            field_or_json_path: Either the underlying :class:`simsopt.field.boozermagneticfield.
                BoozerMagneticField` to be interpolated, or a string path to a JSON file
                containing saved field data.
            degree: the degree of the piecewise polynomial interpolant (required for new fields).
        ...
        if isinstance(field_or_json_path, str):
            ...
            # The C++ object has these values, but they need to be set as Python attributes
            # Use getter methods to access the C++ values
            BoozerMagneticField.__init__(self, self.psi0, self.field_type, self.get_nfp(), self.get_stellsym())
