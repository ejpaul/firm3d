...
from simsopt.util.constants import ALPHA_PARTICLE_CHARGE as CHARGE
from simsopt.util.constants import ALPHA_PARTICLE_MASS as MASS
from simsopt.util.constants import FUSION_ALPHA_PARTICLE_ENERGY as ENERGY
...
def test_derivs(field, sc_particle, nfp, n_metagrid_pts, n_test_pts, verify=True):
    ...
    # generate test points
    r = np.random.uniform(
        low=r_range[0] + 0.1, high=r_range[1] - 0.1, size=(n_test_pts, 1)
    )
    phi = np.random.uniform(low=0, high=2 * np.pi, size=(n_test_pts, 1))
    z = np.random.uniform(
        low=-z_range[1] + 0.1, high=z_range[1] - 0.1, size=(n_test_pts, 1)
    )
    ...
    new_derivs = firm3dpp.test_derivatives_cartesian(
        quad_info,
        r_range,
        phi_range,
        z_range,
    ...
    if verify:
        ...
        for i in range(n_test_pts):
            old_derivs[i, :] = cartesian_rhs(
                rphiz[i, :], vpar_init[i], field, MASS, CHARGE, VELOCITY
            )
...
def test_timestep(field, sc_particle, nfp, n_metagrid_pts, n_test_pts, verify=True):
    ...
    z = np.random.uniform(
        low=z_range[0] + 0.1, high=z_range[1] - 0.1, size=(n_test_pts, 1)
    )
    rphiz = np.hstack((r, phi, z))
    rphiz = np.ascontiguousarray(rphiz)
    ...
    if verify:
        ...
        print(
            f"Maximum relative error in final positions on {n_test_pts} points: {diff}"
        )
        if diff > 1e-6:
