...
TEST_CONFIGS = {
    ...
    "mhd_asym_boozmn": {
        ...
        # Stellsym will be inferred from the file
        "stellsym": True,  # File is actually symmetric
        "order": 3,
    },
}
...
class TestInterpolatedBoozerFieldSaveLoad(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Forced garbage collection, preventing C++ state accumulation
    ...
    def tearDown(self):
        ...
        if (
            TestInterpolatedBoozerFieldSaveLoad._test_count % self._max_tests_before_gc
            == 0
        ):
            ...
            # This collects objects that survived multiple garbage cycles
    ...
    def _verify_attributes(self, field1, field2, config_name):
        ...
        for attr in sorted(common_attrs):
            ...
            try:
                ...
                if attr == "rule":
                    self.assertEqual(
                        v1.degree, v2.degree, f"{config_name}: rule.degree mismatch"
                    )
                ...
                if isinstance(v1, np.ndarray) and isinstance(v2, np.ndarray):
                    ...
                    self.assertEqual(
                        v1, v2, f"{config_name}: {attr} mismatch: {v1} != {v2}"
                    )
        ...
        self.assertEqual(
            len(failed_attrs), 0, f"{config_name}: Failed attributes: {failed_attrs}"
        )
    ...
    def _verify_python_attributes(self, field1, field2, config_name):
        ...
        for attr in python_attrs:
            ...
            v2 = getattr(field2, attr)
            self.assertEqual(
                v1, v2, f"{config_name}: Python attribute {attr} mismatch: {v1} != {v2}"
            )
    ...
    def _verify_cpp_attributes(self, field1, field2, config_name):
        ...
        for attr in status_attrs:
            ...
            self.assertEqual(
                v1, v2, f"{config_name}: C++ status flag {attr} mismatch: {v1} != {v2}"
            )
    ...
    def _verify_evaluations(self, field1, field2, config_name, num_points=1000):
        ...
        points[:, 0] = (
            points[:, 0] * (s_range[1] - s_range[0]) + s_range[0]
        )  # s coordinate
        points[:, 1] = (
            points[:, 1] * (theta_range[1] - theta_range[0]) + theta_range[0]
        )  # theta
        points[:, 2] = (
            points[:, 2] * (zeta_range[1] - zeta_range[0]) + zeta_range[0]
        )  # zeta
        ...
        multi_quantities = [
            ...
            (
                ...
                "status_modB_derivs",
                3,
            ),  # [dmodB/ds, dmodB/dtheta, dmodB/dzeta]
        ...
        for qty_name, status_flag in scalar_quantities:
            if getattr(field1, status_flag):
                ...
                np.testing.assert_allclose(
                    ...
                    rtol=1e-12,
                    atol=1e-14,
                    err_msg=f"{config_name}:{qty_name} mismatch @ {num_points} points",
                )
        ...
        self.assertGreater(
            len(quantities_to_test), 0, f"{config_name}: No quantities were tested!"
        )
    ...
    def _test_config(self, config_name, config_params):
        ...
        try:
            ...
            bri = BoozerRadialInterpolant(
                config_params["file"], config_params["order"], comm=comm
            )
            ...
            # This prevents state accumulation that can cause segfaults
            gc.collect()
    ...
    def test_saveload_mhd_sym_boozmn(self):
        """Finite-beta stellarator symmetric equilibrium (boozmn file #3)."""
        self._test_config("mhd_sym_boozmn", TEST_CONFIGS["mhd_sym_boozmn"])

    def test_saveload_mhd_sym_wout(self):
        """Finite-beta stellarator symmetric equilibrium (wout file #4)."""
    ...
    def test_invalid_reduced_reordered_files(self):
        ...
        for config_name in ["mhd_sym_boozmn", "mhd_asym_boozmn"]:
            ...
            if "reduced_file" in config:
                with self.assertRaises(
                    ValueError, msg=f"{config_name} reduced file should fail"
                ):
                    BoozerRadialInterpolant(
                        config["reduced_file"], config["order"], comm=comm
                    )
        ...
        for config_name in ["mhd_sym_boozmn", "mhd_asym_boozmn"]:
            ...
            if "reordered_file" in config:
                with self.assertRaises(
                    ValueError, msg=f"{config_name} reordered file should fail"
                ):
                    BoozerRadialInterpolant(
                        config["reordered_file"], config["order"], comm=comm
                    )
    ...
    def test_saveload_coarse_resolution(self):
        ...
        self._test_config_simple(
            config,
            [0.3, 0.7, 4],
            [0, np.pi, 4],
            [0, 2 * np.pi / config["nfp"], 8],
            "coarse_resolution",
            num_points=50,
        )
    ...
    def test_saveload_small_domain(self):
        ...
        self._test_config_simple(
            ...
            [0.2, 0.4, 6],
            [0, np.pi / 2, 6],
            [0, np.pi / config["nfp"], 12],
            "small_domain",
            num_points=50,
        )

    def _test_config_simple(
        self, config, s_range, theta_range, zeta_range, test_name, num_points=100
    ):
        """
