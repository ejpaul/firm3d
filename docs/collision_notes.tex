...
A warning is printed if $\lnL < 2$, where the perturbative binary-collision
expansion underlying the Fokker--Planck operator begins to lose validity.
...
The friction (drag) coefficient \cite{trubnikov1965,helander2002}
(ASCOT5: \texttt{Q}) is
\begin{equation}
  Q^{(b)} = -\!\left(1 + \frac{m_a}{m_b}\right)\frac{\Gamma_b\,G(x)}{v^2}.
\end{equation}
The total deterministic drift in speed, including the It\^o correction
arising from the transformation of the Fokker--Planck operator from
Cartesian velocity space to the speed coordinate $v$
...
\begin{equation}
  ...
  = Q^{(b)}
    ...
    \label{eq:K}
\end{equation}
This is ASCOT5's \texttt{mccc\_coefs\_K} \cite{ascot5code} and the drift
coefficient $K$ in Eq.~(32) of Hirvijoki et al.\ \cite{hirvijoki2013}.
Note that $Q$ alone is the physical friction; the remaining two terms are
the It\^o correction $\tfrac{1}{2}\partial_v g_v^2 = \partial_v D_\parallel$
plus the geometric term $2D_\parallel/v$ from the Jacobian $v^2$ of
spherical velocity-space coordinates.
