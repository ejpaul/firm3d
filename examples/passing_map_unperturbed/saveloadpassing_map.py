...
proc0_print(f"  Total attributes: field1={len(attrs1)}, field2={len(attrs2)}, common={len(common_attrs)}")
...
for attr in sorted(common_attrs):
    ...
    try:
        ...
        if attr == 'rule':
            assert v1.degree == v2.degree, f"rule.degree mismatch: {v1.degree} != {v2.degree}"
            np.testing.assert_allclose(v1.nodes, v2.nodes, rtol=1e-12, atol=1e-14, err_msg="rule.nodes mismatch")
            np.testing.assert_allclose(v1.scalings, v2.scalings, rtol=1e-12, atol=1e-14, err_msg="rule.scalings mismatch")
        ...
        if isinstance(v1, np.ndarray) and isinstance(v2, np.ndarray):
            ...
            if attr in ['nfp', 'stellsym', 'field_type', 'psi0', 's_range', 'theta_range', 'zeta_range'] or attr.startswith('status_'):
...
proc0_print(f"    ✓ Verified: {verified_count} ({verified_public} public, {verified_private} private)")
...
assert verified_count >= 30, f"Only {verified_count} attributes verified - expected at least 30"
...
for quantity in quantities_to_test:
    try:
        ...
        # Note: rtol=1e-12 and atol=1e-14 are very strict tolerances (12-14 decimal places!)
        # These ensure the loaded field is bit-for-bit identical to the original
        np.testing.assert_allclose(values1, values2, rtol=1e-12, atol=1e-14,
                                 err_msg=f"{quantity} values don't match between original and loaded fields at all {len(points)} points")
        ...
        proc0_print(f"  ✅ SUCCESS: {quantity} values match perfectly at all {len(points)} points!")
        proc0_print(f"    Statistics - Max diff: {max_diff:.2e}, Mean diff: {mean_diff:.2e}")
        proc0_print(f"    Value range - Min: {np.min(values1):.6f}, Max: {np.max(values1):.6f}")
        
    except Exception as e:
        proc0_print(f"  ❌ FAILED {quantity}: {e}")
        proc0_print(f"    This indicates the loaded field is not working correctly for {quantity}")

proc0_print(f"  📊 COMPREHENSIVE TEST COMPLETED!")
