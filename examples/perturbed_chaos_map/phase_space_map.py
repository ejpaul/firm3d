...
from firm3d.field.trajectory_helpers import PassingPerturbedPoincare, PassingPoincare, compute_peta, MapPhaseSpace, min_volumemodB
from firm3d.field.boozermagneticfield import ShearAlfvenHarmonic, ShearAlfvenWave, ShearAlfvenWavesSuperposition
import time 
from firm3d.field.tracing_helpers import initialize_position_profile, initialize_position_uniform_vol, initialize_position_uniform_surf
...
harmonics = {harmonic:{}}
max_ell = 2
# list of possible line styles for the resonance lines, indexed by ell value (shifted by max_ell to avoid negative indices)
#  ell =                 -2           -1      0           1           2
...
if perfect: bri_p, field_p = bri, field
else:
...
if perfect: filepath = f'qa_{Phim}_{Phin}_{dir}'
else: filepath = f'bqa_{Phim}_{Phin}_{dir}'
...
for mu_h in mu_harmonics:  
    # compute rotational profile for given pitch angle
    profile = compute_rotational_profile(mu_h/(min_volmodB * Ekin), sign_vpar, False, comm=comm)
    perturbed_pitch_angle.append(sign_vpar * np.abs(mu_h)/ Eprime)
    drift_helicity = profile[:,3]
    radial_position = profile[:,0]

    if verbose and (plot_counter % 5 == 0): # only plot some pitch angles to avoid overcrowding
        plt.plot(radial_position, drift_helicity, label=f"$\\lambda$={sign_vpar * np.abs(mu_h)/ Eprime:.2f}")

    plot_counter += 1

    # creates a dictionary of the form {harmonic: {ell: [[resonance_peta], [resonance_radius]]}} 
    ...
    for h in harmonics.keys():
        ...
        for ell in range(-max_ell,max_ell+1):
            h_res = calculate_QS_resonance(Phim_h,Phin_h,helicity_M,helicity_N,omega,np.mean(profile[:,2]), ell=ell)
            crossings = calculate_crossings(drift_helicity, h_res, radial_position)
            if len(crossings) != 0:
                for crossing_index, radius in enumerate(crossings):
                    if ell in harmonics[h].keys():
                        # if the resonance location passes through the rotational profile multiple times, 
                        # we want to store all of the crossing locations.
                        # if this is the first entry, make empty lists
                        if crossing_index > (len(harmonics[h][ell])-1):
                            harmonics[h][ell].append([[],[]])
                        harmonics[h][ell][crossing_index][0].append(sign_vpar * np.abs(mu_h)/ Eprime)
                        harmonics[h][ell][crossing_index][1].append(radius)
                    else:
                        harmonics[h][ell] = [[[sign_vpar * np.abs(mu_h)/ Eprime], [radius]]]

    if verbose:
        # plot the rotational profile
        plt.tight_layout()
        plt.legend(fontsize=14, markerscale=1.5)
        plt.savefig(filepath + f"harmonic_profile.png", dpi=400)
    if comm is not None:
        comm.Barrier()
...
ell_rad_list = []
lines_modes = []
profile = compute_rotational_profile(lam, sign, True, comm)
for ell in range(-max_ell,max_ell+1):
    h_res = calculate_QS_resonance(Phim,Phin,helicity_M,helicity_N,omega,np.mean(profile[:,2]), ell=ell)
    crossings = calculate_crossings(profile[:,3], h_res, profile[:,0])
    for radius in crossings:
        ell_rad_list.append((ell, radius))
...
if verbose:
    ...
    # create list of line styles for each resonance line based on the ell value
    main_harmonic_linestyles = [possible_linestyles[l[0]+max_ell] for l in ell_rad_list]

    # create subplots
    fig, (ax_left,ax_center, ax_right, ax_dummy) = plt.subplots(1, 4,
        gridspec_kw={"width_ratios": [1, 4, 4, 0.61]},
        figsize=(34,12)) 
    

    # plot the perturbation amplitude as a function of radius on the left
    ax_left.plot(Phihat[1][:-1], Phihat[0])
    filename = filepath + f"poincare.png"

    # plot the resonance lines on the pertubation magnitude plot
    for i, elem in enumerate(ell_rad_list):
        ell, radius = elem
        print(f'{i}: ell={ell}, arr={radius}')
        print(f'linecolor: {linecolors}, linestyle: {main_harmonic_linestyles}')
        ax_left.plot([min(Phihat[1]), max(Phihat[1])], [radius, radius], lw=7, color=linecolors[0], linestyle=possible_linestyles[ell+3])

    ax_left.set_xlabel(r"$\delta \phi$ [V]")
    ax_left.set_ylabel(r"$s_0$") 

    # plot the poincare plot in the center, with resonance lines
    ax_center, lines_colors = map.plot_poincare(ax = ax_center, 
                                        filename=filename,
                                        lines=ell_rad_list,
                                        legend_plot = False,
    ...
    ax_center.set_title(rf'$\lambda$ = {lam * sign}')
    ax_center.sharey(ax_left)
    
    print('Finished plotting poincare ', flush=True)

    # plot heatmap
    ax_right = heat_map.plot_surfaces(ax = ax_right, savepath = filepath + f"heatmap.png", plot_losses=plot_losses)
    ...
    for h in harmonics.keys():
        ...
        for ell in harmonics[h].keys():
            ...
            for crossing_line_index, crossing_line in enumerate(harmonics[h][ell]):
                ...
                line, = ax_right.plot(resonance_pitch[:sign_changes[0]], 
